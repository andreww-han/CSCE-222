...
\end{problem}
\begin{solution} 
\textbf{b)} 1. $ x^{2} - 7x + 10$ \\ 2. $x_{1} = 5, x_{2} = 2$ \\ 3. $$ a_{n} = \alpha 5^{n} + \beta 2^{n} $$ $$ a_{0} = \alpha + \beta = 2 $$ $$ a_{1} = 5 \alpha + 2 \beta = 1$$ $$\alpha = -1, \beta = 3$$ $$ a_{n} = -(5)^{n} + 3*(2)^{n}$$
\textbf{c)} 1. $ x^{2} -6x + 8$ \\ 2. $x_{1} = 4, x_{2} = 2$ \\ 3. $$ a_{n} = \alpha 4^{n} + \beta 2^{n}$$ $$ a_{0} = \alpha + \beta = 4$$ $$ a_{1} = 4 \alpha + 2 \beta = 10 $$ $$ \alpha = 1, \beta = 3$$ $$ a_{n} = 4^{n} + 3*2^{n}$$
\textbf{d)} 1. $x^{2} -2x + 1$ \\ 2. $x_{1} = 1$ \\ 3. $$ a_{n} = \alpha (1)^{n} + n \beta (1)^{n}$$ $$ a_{0} = \alpha = 4$$ $$ a_{1} = \alpha + \beta = 1$$ $$ \alpha = 4, \beta = -3$$ $$ a_{n} = 4 -3n$$
\end{solution}
...
\end{problem}
\begin{solution} 
\textbf{a)} (1,4), (2,3), (3,2), (4,1)\\
\textbf{b)} (1,2), (1,3), (1,4), (1,5), (2,3), (2,4), (2,5), (3,4), (3,5), (4,5)\\ 
\textbf{c)} (1,2), (1,3), (1,4), (2,3), (2,4), (3,4) 
\end{solution}
...
\end{problem}
\begin{solution} 
\textbf{a)} Not reflexive because if $ 1,1 \in \R, 1+1 \ne 0$ \\ Symmetric because x + y = 0 is the same as y + x =0 \\ Not anti-symmetric because if $1, -1 \in \R, 1 - 1 =0 \text{ and } -1 + 1 = 0, \text { but } 1 \ne -1$ \\ Not transitive because if $1,-1 \in \R$ and $ -1,1 \in \R, 1,1 \in \R$ is NOT true as shown above. \\
\textbf{b)} Reflexive because if $x,x \in \R$, then $x = \pm x$ will have to be true. \\ Symmetric because if $x,y \in \R$ and $x = \pm y$, then $y,x \in \R$ and $y = \pm x$ \\ Not anti-symmetric because if $1,-1 \in \R$ and $-1,1 \in \R$, $1 \ne -1$ which means the implication that 1 = -1 is not true. \\ Transitive because if $x = \pm y$ and $y = \pm z$, then $x = \pm z$
\end{solution}

\begin{problem} (5 points $\times$ 2 = 10 points)
Let $A$ be the set of all people and $(x,y)\in A\times A$.  Is each of the following an equivalence relation?  For each subproblem, explain which 
of the three properties of an equivalence relation -- reflexivity, symmetry, and transitivity -- are satisfied and which are not, by explaining why or why not.Then, answer whether it is an equivalence relation.
\begin{enumerate}[a)]
...
We define on the set $\N_1=\{1,2,3,\cdots\}$ of positive integers a relation $\sim$ such that two positive integers $x$ and $y$ satisfy $x\sim y$ if and only if $x/y=2^k$ for some integer $k$. Show that $\sim$ is an equivalence relation.
\end{problem}
\begin{solution} 
...
For each subproblem, explain which of the three properties of a partial ordering -- reflexivity, antisymmetry, and transitivity -- are satisfied and which are not, by explaining why or why not.  Then, answer whether it is a partial ordering.
\end{problem}
